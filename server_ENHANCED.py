#!/usr/bin/env python3
"""
SlideGen Pro - ENHANCED EDITION - Backend Server with Authentication & Subscriptions
...
def call_anthropic(prompt, max_tokens=2000, retry_count=0, max_retries=3):
    """Make API call to Anthropic with retry logic for overload errors"""
    import time
    ...
    try:
        ...
        elif response.status_code == 529 and retry_count < max_retries:
            # Overloaded - retry with exponential backoff
            wait_time = (2 ** retry_count) * 2  # 2, 4, 8 seconds
            logger.warning(f"API overloaded (529), retrying in {wait_time}s (attempt {retry_count + 1}/{max_retries})")
            time.sleep(wait_time)
            return call_anthropic(prompt, max_tokens, retry_count + 1, max_retries)
        else:
            raise Exception(f"API error: {response.status_code} - {response.text}")

    except Exception as e:
        if "529" not in str(e) or retry_count >= max_retries:
            logger.error(f"Anthropic API error: {str(e)}")
            raise
        # Retry for other temporary errors
        wait_time = (2 ** retry_count) * 2
        logger.warning(f"API error, retrying in {wait_time}s (attempt {retry_count + 1}/{max_retries})")
        time.sleep(wait_time)
        return call_anthropic(prompt, max_tokens, retry_count + 1, max_retries)
...
{{
  ...
  "title_font": "Font name - BE CREATIVE! Choose from: 'Montserrat', 'Playfair Display', 'Bebas Neue', 'Oswald', 'Raleway', 'Poppins', 'Lora', 'Merriweather', 'Roboto Slab', 'Century Gothic', 'Futura', 'Avenir', 'Helvetica Neue', 'Garamond', 'Didot', 'Rockwell', 'Impact', 'Trebuchet MS', or other unique fonts",
  "body_font": "Font name - Pair well with title! Choose from: 'Open Sans', 'Roboto', 'Lato', 'Source Sans Pro', 'Nunito', 'Karla', 'Work Sans', 'PT Sans', 'Cabin', 'Proxima Nova', 'Gotham', 'Avenir', 'Helvetica Neue', 'Georgia', 'Palatino', 'Baskerville', 'Book Antiqua', or other readable fonts",
  "title_size": 40,
  "body_size": 20,
  "style_description": "2-3 sentence description explaining the creative design vision and how it incorporates image placeholders",
  "mood": "One word (e.g. bold, striking, sophisticated, vibrant, dramatic, sleek, artistic, dynamic, refined, cutting-edge)",
...
🎨 BE BOLD AND CREATIVE! Don't default to boring corporate blues and Arial!
- Explore unique color combinations that stand out (deep purples, rich teals, warm oranges, sophisticated burgundies, modern forest greens)
- Try dramatic contrasts (dark backgrounds with bright text, or vice versa)
- Consider gradient-friendly colors, complementary color schemes, or monochromatic sophistication
- Mix modern sans-serif titles with elegant serif bodies, or pair bold display fonts with clean sans-serifs
- Think about the user's industry/mood and push creative boundaries while staying professional

FONT PAIRING EXAMPLES (Title → Body):
✨ MODERN & BOLD: Montserrat → Open Sans, Oswald → Lato, Bebas Neue → Roboto
✨ ELEGANT & SOPHISTICATED: Playfair Display → Lora, Didot → Garamond, Cormorant → Crimson Text
✨ CREATIVE & DYNAMIC: Raleway → Nunito, Poppins → Work Sans, Quicksand → Karla
✨ TECH & SLEEK: Roboto Slab → Roboto, Futura → Avenir, Space Grotesk → Inter
✨ ARTISTIC & UNIQUE: Abril Fatface → Karla, Archivo Black → Source Sans Pro, Righteous → Cabin

COLOR SELECTION GUIDELINES:
- Choose unexpected, beautiful color combinations that match the requested mood
- Ensure text_color has STRONG contrast with background_color (readable from distance)
- Accent color for image borders should be striking and memorable
- Consider trending color palettes, not just safe defaults
- Examples: Deep navy (#1A237E) + gold (#FFD700), Emerald (#2ECC71) + cream (#FFF8DC), Plum (#8E44AD) + mint (#F0FFF0)

UNIQUE STYLE INSPIRATIONS:
- Bold Editorial: Black backgrounds, Playfair Display titles, gold accents, large serif fonts
- Modern Minimalist: Off-white background, Montserrat, single bright accent color, clean lines
- Tech Innovation: Dark blue/purple gradient-friendly colors, Poppins, neon accents
- Creative Agency: Vibrant colors (coral, teal, yellow), mixed font styles, playful borders
- Luxury Brand: Deep colors (burgundy, forest green, navy), Didot/Garamond, gold accents
- Startup Energy: Bold colors (electric blue, lime green), Oswald/Raleway, dynamic layouts
...
def list_themes():
    """List all available themes with their preview images"""
    try:
        themes_dir = 'theme-previews'
        themes = []

        if not os.path.exists(themes_dir):
            logger.warning(f"Theme previews directory not found: {themes_dir}")
            return jsonify({'themes': []})

        # Scan theme-previews directory for theme folders
        for theme_name in os.listdir(themes_dir):
            theme_path = os.path.join(themes_dir, theme_name)

            # Skip non-directories and README
            if not os.path.isdir(theme_path) or theme_name.startswith('.'):
                continue

            # Find all preview images in this theme folder
            previews = []
            for filename in sorted(os.listdir(theme_path)):
                if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')):
                    # URL will be relative to the HTTP server root
                    preview_url = f'/theme-previews/{theme_name}/{filename}'
                    previews.append(preview_url)

            # Only add theme if it has at least one preview image
            if previews:
                themes.append({
                    'name': theme_name,
                    'previews': previews
        ...
        logger.info(f"Found {len(themes)} themes with preview images")
...
def generate_pptx():
    ...
    try:
        ...
        # SKIPPING AI speaker notes generation for faster downloads
        # The pptx_generator will use template-based notes instead
        logger.info("Using template-based speaker notes for fast generation...")
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("🚀 SlideGen Pro ENHANCED EDITION")
    print("=" * 60)
    ...
    print("✓ Image placeholders on all content slides (auto-generated)")
    print("=" * 60)

    # For local development - ENHANCED VERSION RUNS ON PORT 5000
